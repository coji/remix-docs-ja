...
export class BM25SearchEngine {
    ...
    return new Promise((resolve, reject) => {
      ...
      import('kuromoji')
        .then((kuromojiModule) => {
          ...
          // Get the path to kuromoji dict directory (use local node_modules symlink)
          const defaultDictPath = tokenizerPath || 'node_modules/kuromoji/dict'

          kuromoji
            .builder({
              dicPath: defaultDictPath,
            })
  ...
  private calculateBM25Score(docId: string, queryTerms: string[]): number {
    ...
    for (const term of queryTerms) {
      ...
      // IDF calculation
      const idf = Math.log(
        (this.index.totalDocuments - docFreq + 0.5) / (docFreq + 0.5),
      )
      ...
      const tf =
        (termFreqInDoc * (K1 + 1)) /
        (termFreqInDoc +
          K1 * (1 - B + B * (doc.length / this.index.averageDocumentLength)))
  ...
  private findHighlights(content: string, queryTerms: string[]): string[] {
    ...
    for (const term of queryTerms) {
      const matchingTokens = contentTokens.filter(
        (token) => token.includes(term) || term.includes(token),
      )
    ...
    return JSON.stringify(
      {
        ...
        termFrequency: Object.fromEntries(
          Array.from(this.index.termFrequency.entries()).map(
            ([term, docMap]) => [term, Object.fromEntries(docMap.entries())],
          ),
        ),
        documentFrequency: Object.fromEntries(
          this.index.documentFrequency.entries(),
        ),
